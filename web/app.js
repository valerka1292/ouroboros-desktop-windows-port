...
function initSettings() {
    ...
    page.innerHTML = `
        ...
        <div class="settings-scroll">
            <div class="form-section">
                ...
                <div class="form-row"><div class="form-field"><label>Base URL</label><input id="s-openai-compat-base" placeholder="https://openrouter.ai/api/v1"></div></div>
                <div class="form-row"><div class="form-field"><label>API Key</label><input id="s-openai-compat-key" type="password" placeholder="sk-..."></div></div>
                <div class="form-row"><div class="form-field"><label>OpenAI API Key (optional, web_search fallback)</label><input id="s-openai" type="password"></div></div>
                <div class="form-row"><div class="form-field"><label>Anthropic API Key (optional, Claude Code tool)</label><input id="s-anthropic" type="password"></div></div>
            </div>
            <div class="divider"></div>
            <div class="form-section">
                <h3>Models</h3>
                <div class="form-row"><div class="form-field"><label>Main Model</label><input id="s-model" value="anthropic/claude-sonnet-4.6" style="width:250px"></div></div>
                <div class="form-row"><div class="form-field"><label>Code Model</label><input id="s-model-code" value="anthropic/claude-sonnet-4.6" style="width:250px"></div></div>
                <div class="form-row"><div class="form-field"><label>Light Model</label><input id="s-model-light" value="google/gemini-3-flash-preview" style="width:250px"></div></div>
                <div class="form-row"><div class="form-field"><label>Fallback Model</label><input id="s-model-fallback" value="google/gemini-3-flash-preview" style="width:250px"></div></div>
                <div class="form-row"><div class="form-field"><label>Claude Code Model</label><input id="s-claude-code-model" value="sonnet" placeholder="sonnet, opus, or full name" style="width:250px"></div></div>
            </div>
            <div class="divider"></div>
            <div class="form-section">
                <h3>Runtime</h3>
    ...
    fetch('/api/settings').then(r => r.json()).then(s => {
        if (s.OPENAI_COMPAT_BASE_URL) document.getElementById('s-openai-compat-base').value = s.OPENAI_COMPAT_BASE_URL;
        if (s.OPENAI_COMPAT_API_KEY) document.getElementById('s-openai-compat-key').value = s.OPENAI_COMPAT_API_KEY;
    ...
    document.getElementById('btn-save-settings').addEventListener('click', async () => {
        ...
        const compatBase = document.getElementById('s-openai-compat-base').value.trim();
        if (compatBase) body.OPENAI_COMPAT_BASE_URL = compatBase;
        const compatKey = document.getElementById('s-openai-compat-key').value;
        if (compatKey && !compatKey.includes('...')) { body.OPENAI_COMPAT_API_KEY = compatKey; body.OPENROUTER_API_KEY = compatKey; }
